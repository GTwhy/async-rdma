...
impl RemoteMrManager {
    ...
    pub(crate) fn new() -> Self {
        ...
        let handler = tokio::task::spawn(main(timer_rx, RmrMap::clone(&mr_own)));
        Self {
            mr_map: mr_own,
            timer_tx,
            handler,
    ...
    pub(crate) async fn release_mr(&self, token: &MrToken) -> io::Result<LocalMr> {
        debug!("release_mr {:?}", token);
        remove_mr_from_map(&self.mr_map, token)
            .await
            .map_or_else(Err, |(lmr, release_tx)| {
                // cancel timer task
...
async fn remove_mr_from_map(
    map: &RmrMap,
    token: &MrToken,
) -> io::Result<(LocalMr, oneshot::Sender<()>)> {
    map.lock().await.remove(token).map_or_else(
        || {
            Err(io::Error::new(
                io::ErrorKind::InvalidInput,
...
async fn main(mut timer_rx: mpsc::Receiver<MrToken>, map: RmrMap) {
    loop {
        ...
        remove_mr_from_map(&map, &token).await.map_or_else(
            |err| {
