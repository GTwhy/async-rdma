...
impl MrAllocator {
    ...
    pub(crate) unsafe fn alloc(self: &Arc<Self>, layout: &Layout) -> io::Result<LocalMr> {
        let inner = self.alloc_inner(layout, false)?;
    ...
    pub(crate) fn alloc_zeroed(self: &Arc<Self>, layout: &Layout) -> io::Result<LocalMr> {
        ...
        let inner = unsafe { self.alloc_inner(layout, true)? };
        Ok(LocalMr::new(inner))
    }
    ...
    pub(crate) unsafe fn alloc_inner(
        ...
        self.alloc_from_je(layout, zeroed).map_or_else(||{
    ...
    unsafe fn alloc_from_je(&self, layout: &Layout, zeroed: bool) -> Option<*mut u8> {
        let addr = {
            if zeroed {
                tikv_jemalloc_sys::mallocx(
                    layout.size(),
                    (MALLOCX_ALIGN(layout.align())
                        | MALLOCX_ARENA(self.arena_ind.cast())
                        | MALLOCX_ZERO)
                        .cast(),
                )
            } else {
