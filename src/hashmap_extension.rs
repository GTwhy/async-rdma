...
pub(crate) trait HashMapExtension<K: Eq + Hash + Copy, V> {
    ...
    fn insert_until_success<F: FnOnce() -> K + Copy>(&mut self, key: K, value: V, key_gen: F) -> K;
}
...
fn insert_until_success_test() {
    ...
    assert_eq!(
        contributors.insert_until_success(1_i32, "GTwhy", key_gen),
        5_i32
    );
    assert_eq!(
        contributors.insert_until_success(8_i32, "gitter-badger", key_gen),
        8_i32
    );
}
