...
impl MrAllocator {
    ...
    fn lookup_raw_mr(&self, addr: usize) -> Arc<RawMemoryRegion> {
        lookup_raw_mr(self.default_arena_ind, addr).map_or_else(
            || {
...
unsafe fn alloc_raw_mem(layout: &Layout, flag: i32) -> Option<*mut u8> {
    let addr = if flag == MALLOCX_ZERO {
        // // SAFETY: dst is valid for writes of `layout.size() * size_of<u8>` bytes and has been properly aligned.
        // std::ptr::write_bytes(addr, 0_u8, layout.size());
        alloc(*layout)
    } else {
        alloc_zeroed(*layout)
    };
